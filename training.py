...
"""os, math, random, dataclass: standard Python utilities.

torch, nn, F: PyTorch core, neural network modules, and common functions (like mse_loss).

Dataset, DataLoader: PyTorch’s dataset and batching utilities.

load_dataset: HuggingFace datasets to fetch nlphuji/flickr30k.

CLIPTokenizer: tokenizer used with CLIP text encoder (same as in your generation pipeline).

tqdm: progress bar for loops.

model_loader: your custom module that loads the pretrained SD-like models (clip, encoder, diffusion, decoder)"""
# -----------------------------
# Config
# -----------------------------
...
MAX_TEXT_LENGTH = 77 matches CLIP’s sequence length.

NUM_TRAIN_TIMESTEPS = 1000: standard DDPM schedule length — number of discrete noise steps."""
...
class Flickr30kDiffusionDataset(Dataset):
    ...
    def __init__(self, hf_split, tokenizer: CLIPTokenizer, image_size: int = 512, max_length: int = 77):
        ...
        # hf_split: the HF dataset split (ds["train"]).
...
truncation=True → cut longer captions.
